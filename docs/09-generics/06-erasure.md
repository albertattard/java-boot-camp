...
Our generic class will be compiled and the type parameter will be replaced by the `Object` type, as we saw [before](#how-are-type-parameters-replaced).  Our code will look like the following.
...
Now consider another class that extends our generic type `Box`.
...
The child class does not have such method.  **How will polymorphisim works when generics are used?**
...
The `put()` method is overloaded.  If we analyse the bytecode of the overloaded method (the method generated by the type erasure), will find that it simply casts the input to `String` and calls our method.
...
Type erasure is creating a new method, that simply calls our method while ensuring polymorphisim, as shown next.
...
After type erasue, the `StringBox` will have two methods.
...
The above example makes use of raw types.  This means that the compiler will not provide us with type checks and we can try to put anything we want our box.  In our example, this will fail, but it some cases this will work and will only fail when we read the item of the box.  The above example fails at runtime with a [`ClassCastException`](https://docs.oracle.com/en/java/javase/14/docs/api/java.base/java/lang/ClassCastException.html).
...
In this example, we are calling the `put()` that takes an `Object` (as the variable `item` is of type object despite the fact that we are pointing to a `String`).  This will type cast the reference and then calls the `put()` method that takes a `String`, as shown next.

![Raw Types]({{ '/assets/images/Raw-Types-Generics-Call-1.png' | absolute_url }})

When using raw types we can access the `put()` method that takes a `String` directly by passing a `String`, as shown next.
...
The `clear()` method invokes the `put(Object)` method.  The subtype, `StringBox`, has its own of `put(Object)` which is generated by the type erasure.  The generated method calls our method `put(String)`, which calls the parent's method as shown next.
